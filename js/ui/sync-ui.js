...
(function (root) {
  ...
  /** Offer the cloud drives found on this computer (iCloud Drive first), or any folder. */
  async function pickFolder(b) {
    const found = await b.detect().catch(() => []);
    if (!found.length) return b.choose();
    const pick = await modal({ title: 'Where should ColdLoad Pro sync?', wide: true, body: h('div', {},
      h('p', {}, 'These cloud drives are installed on this computer. Choose the same one on your other computer. ColdLoad Pro creates a folder “ColdLoad Pro Sync” inside it.'),
      h('ul', { class: 'notes' }, found.map((f) => h('li', {}, h('b', {}, f.provider), ` — ${f.path}`, f.existing ? h('span', { class: 'muted' }, ' (already contains ColdLoad Pro data)') : null)))),
    actions: [{ label: 'Cancel', value: null }, { label: 'Other folder…', value: '__other' }, ...found.slice(0, 3).map((f, i) => ({ label: `Use ${f.provider}`, value: f.path, kind: i === 0 ? 'primary' : 'secondary' }))] });
    if (pick === '__other') return b.choose();
    return pick || null;
  }

  async function setup() {
    const b = bridge();
    const dir = await pickFolder(b);
    if (!dir) return;
  ...
  async function card() {
    ...
    const intro = h('p', { class: 'muted small' }, 'Use the same projects on your Windows PC and your Mac. Set up sync and pick iCloud Drive (or OneDrive, Dropbox, Google Drive) on both computers — ColdLoad Pro finds them automatically. Work done on one computer appears on the other within a few minutes. User accounts stay separate on each computer.');
    if (!cfg.enabled) {
